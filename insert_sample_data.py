...
DATABASE_URL = "postgresql://exam_registration_db_yj6m_user:qUrbvBbnOccssSvbRrAk4dtpugBRiMcK@dpg-d8hqlhe7r5hc73c6emm0-a.ohio-postgres.render.com/exam_registration_db_yj6m"
...
def insert_sample_data():
    ...
    for s in students:
        try:
            ...
            if img is None:
                print(f"Could not read image: {s[6]}")
                continue

            img_resized = cv2.resize(img, (100, 100))
            embedding = json.dumps(
                img_resized.flatten().tolist()
            )
